...
export function handleConfValue(message: ProtocolMessage) {
    ...
    for (const item of infolist) {
        ...
        if (merged.full_name)
            config[String(merged.full_name)] = String(merged.value || "");
    }
    ...
    for (const bufferMsg of bufferInfos) {
        ...
        if (workingBuffers[bufferId]) {
            ...
            if (buffer.type === "server") {
                ...
                serverDeltas[serverKey] =
                  (serverDeltas[serverKey] || 0) + buffer.unread + buffer.notification;
            }
            workingBuffers[bufferId] = buffer;
            console.debug(
                "[handler]   created buffer:",
                buffer.id,
                buffer.shortName,
...
function handleBufferUpdate(
    ...
    const hidden =
      message.hidden !== undefined ? !!message.hidden : buffer.hidden;

    const bufferType =
      message.type !== undefined ? message.type : buffer.bufferType;
...
function formatNotificationBody(lineMsg: BufferLineMessage): string {
    ...
    const prefixText = stripControlCodes(
        prefixParts.map((p) => p.text).join(""),
    ).trim();
...
export function handleBufferLineAdded(message: ProtocolMessage) {
    ...
    for (const lineMsg of lines) {
        ...
        if (!buffer) {
            ...
            if (DEBUG_HANDLERS)
                console.log(
                    "[handler] creating buffer for:",
                    lineMsg.buffer,
                    "nick:",
                    inferredNick,
                    "msg:",
                    lineMsg.message?.substring(0, 50),
                );

            // Auto-create query buffer for incoming PM.
            // Use type:2 (WeeChat query buffer) and notify:3 (notify on everything).
            // Provide local_variables.type='query' so createBuffer sets correct string type.
            // Note: _buffer_opened may overwrite this later with richer metadata,
            // but having correct type here ensures notification logic works immediately.
            buffer = createBuffer({
                ...
                type: 2,
                notify: 3,
                local_variables: { type: "query", plugin: "weechat", server: "" },
            });
        ...
        if (!buffer) {
            console.warn(
                "[handler] buffer unexpectedly undefined for:",
                lineMsg.buffer,
            );
            continue;
        }
        ...
        if (line.displayed) {
            ...
            if (buffer.lastSeen >= 0 || !isSyncing()) {
                ...
                // Requires buffer.notify != 0 and either a highlight or private/query message.
                // WeeChat uses 'query' type for PM buffers, not 'private'.
                ...
                if (
                    buffer.notify >= lineMsg.notify_level &&
                  lineMsg.notify_level >= 1 &&
                  buffer.id !== activeId
                ) {
...
export function handleBufferLineDataChanged(message: ProtocolMessage) {
    const lineMsg = message.objects?.[0]?.content?.[0] as
      | BufferLineMessage
      | undefined;
    if (!lineMsg) {
    ...
    if (!buffer) {
        console.debug(
            "[handler] _buffer_line_data_changed: buffer not found for",
            lineMsg.buffer,
        );
        return;
    }
    ...
    if (matches.length === 0) {
        console.debug(
            "[handler] _buffer_line_data_changed: no matching line for date",
            lineMsg.date,
        );
        return;
    }
...
export function handleBufferMergedOrUnmerged(message: ProtocolMessage) {
    ...
    for (const id in current) {
        ...
        if (
            newNumber > oldNumber &&
          buf.number > oldNumber &&
          buf.number <= newNumber
        ) {
            num -= 1;
        } else if (
            newNumber < oldNumber &&
...
export function handleUpgradeEnded() {
    if (DEBUG_HANDLERS)
        console.log(
            "[handler] _upgrade_ended: WeeChat upgrade complete, reconnecting",
        );
...
export function injectDateChangeMessageIfNeeded(
    buffer: BufferData,
    manually: boolean,
    oldDate: Date,
    newDate: Date,
    ...
    content += newDate.toLocaleDateString(undefined, {
        month: "long",
        day: "numeric",
    });
...
export function handleBufferOpened(message: ProtocolMessage) {
    ...
    const newBuffer = createBuffer(bufferMsg);
    const existingBuffer = getBuffer(newBuffer.id);

    if (existingBuffer) {
        // Buffer already exists (e.g., auto-created by handleBufferLineAdded for an incoming PM).
        // Update metadata from _buffer_opened while preserving runtime state:
        // lines, unread counts, localUnread, lastSeen.
        if (DEBUG_HANDLERS)
            console.log(
                "[handler] merging _buffer_opened into existing buffer:",
                newBuffer.shortName,
            );
        buffers.update((current) => ({
            ...current,
            [newBuffer.id]: {
                ...
                notification: Math.max(
                    existingBuffer.notification,
                    newBuffer.notification,
                ),
                nicklist: { ...existingBuffer.nicklist },
            },
        }));
    } else {
        addBuffer(newBuffer);
    ...
    if (newBuffer.type === "server") {
        const key = `${newBuffer.plugin}.${newBuffer.server}`;
        servers.update((current) => ({
            ...current,
            [key]: { id: newBuffer.id, unread: 0 },
        }));
    } else {
        const serverKey = `${newBuffer.plugin}.${newBuffer.server}`;
        const delta = newBuffer.unread + newBuffer.notification;
        if (delta > 0) {
            servers.update((current) => {
                ...
                return {
                    ...current,
                    [serverKey]: { ...server, unread: server.unread + delta },
                };
            });
        }
    }
    ...
    if (
        targetNick &&
      newBuffer.shortName.toLowerCase() === targetNick.toLowerCase()
    ) {
        if (DEBUG_HANDLERS)
            console.log("[handler] auto-switching to query buffer for:", targetNick);
        setActiveBuffer(newBuffer.id);
...
export function handleBufferRenamed(message: ProtocolMessage) {
    ...
    const updated = updateBuffer(bufferId, {
        ...
        prefix: ["#", "&", "+"].includes(obj.short_name.charAt(0))
...
export function handleBufferLocalvarChanged(message: ProtocolMessage) {
    ...
    const indent = ["channel", "private"].includes(type);
    const serverSortKey =
      `${plugin}.${server}${type === "server" ? "" : "." + buffer.shortName}`.toLowerCase();
    ...
    const updated = updateBuffer(bufferId, {
        ...
        serverSortKey,
        pinned,
        localVariables: mergedLocalVars,
    });
    if (!updated) return;
    // Use update() to merge with current store state, preventing overwrites
    // of concurrent changes from other handlers.
...
export function handleHotlistInfo(message: ProtocolMessage) {
    ...
    for (const id of hotlistBufferIds) {
        ...
        if (
            (freshBuf.unread || 0) +
              (freshBuf.notification || 0) +
              (freshBuf.localUnread || 0) >
          0
        )
            continue;
...
function clearBufferNicklist(bufferId: string) {
    ...
    if (buffer) {
        ...
        buffer.nicklist = {
            root: buffer.nicklist.root || { name: "", visible: "", nicks: [] },
        };
    }
}
...
export function handleNicklist(message: ProtocolMessage, fresh?: boolean) {
    ...
    if (DEBUG_NICKLIST)
        console.log(
            "[nicklist] handleNicklist called, total items:",
            nicklist?.length ?? 0,
            "fresh:",
            !!fresh,
        );
    ...
    for (const n of nicklist) {
        ...
        if (DEBUG_NICKLIST)
            console.log(
                "[nicklist] processing item for buffer",
                buffer.shortName || buffer.id,
                "- pointers:",
    ...
    if (DEBUG_NICKLIST) {
        ...
        for (const id of modifiedBuffers) {
            ...
            if (buf) {
                ...
                const nickCounts = groups
                    .map((g) => `${g}:${buf.nicklist?.[g]?.nicks?.length ?? 0}`)
                    .join(", ");
                summary.push({
                    buffer: buf.shortName,
                    groups: groups.join(", "),
                    nicks: nickCounts,
                });
            }
    ...
    if (DEBUG_NICKLIST)
        console.log(
            "[nicklist] updating buffers store with",
            modifiedBuffers.size,
            "modified buffer(s)",
        );
    // Use update() to merge with current store state, preventing overwrites
    // of concurrent changes from other handlers.
...
// Nick change "is now known as" infolines are sent by WeeChat via _buffer_line_added
// and handled by handleBufferLineAdded — this handler only updates nicklist state.
export function handleNicklistDiff(message: ProtocolMessage) {
    ...
    if (DEBUG_NICKLIST)
        console.log(
            "[nicklist] handleNicklistDiff called, total items:",
            nicklist?.length ?? 0,
        );
    if (!nicklist) return;
    ...
    if (DEBUG_NICKLIST) {
        ...
        for (const bid of affectedBufferIds) {
            const buf = store[bid];
            const count = buf
                ? Object.values(buf.nicklist).reduce((s, g) => s + g.nicks.length, 0)
                : -1;
            console.log(
                "[nicklist] BEFORE diff:",
                buf?.shortName || bid,
                "- nicks:",
                count,
            );
    ...
    for (const n of nicklist) {
        ...
        if ((n as NickGroupMessage).group === 1) {
            ...
            if (oldIdx >= 0) {
                nickGroup.nicks.splice(oldIdx, 1);
                modifiedBuffers.add(ptr0);
            }
        }
    }
    ...
    for (const n of nicklist) {
        ...
        if (!("root" in buffer.nicklist)) {
            if (DEBUG_NICKLIST)
                console.log(
                    "[nicklist] skipping diff for",
                    buffer.shortName || buffer.id,
                    "- nicklist not requested",
                );
            continue;
        }
...
export function handleLineInfo(
    message: ProtocolMessage,
    manually: boolean = true,
    clearLinesBufferId?: string,
) {
    const lines = message.objects[0]?.content as BufferLineMessage[];
    if (!lines) {
    ...
    console.debug(
        "[handler] handleLineInfo: manually=" +
          manually +
          " lines=" +
          lines.length +
          " displayed=" +
