...
// Use fflate for zlib decompression (WeeChat binary protocol)
// fflate has ESM exports and works in both Node.js and browser environments
// Try to load fflate as a global first, then fall back to dynamic import
const fflate = globalThis.fflate || (typeof window !== 'undefined' ? window.fflate : null);
const unzlibSync = fflate ? fflate.unzlibSync : null;

/**
 * Decompress a zlib-compressed buffer using fflate.
 * This replaces the synchronous Zlib.Inflate API from zlibjs.
 */
function decompressZlib(raw) {
    if (!unzlibSync) {
        throw new Error('fflate not loaded - zlib decompression unavailable');
    }
    try {
...
/**
 * Formats a localvar set command to pin/unpin a buffer.
 * https://weechat.org/files/doc/stable/weechat_relay_protocol.en.html#command_input
 * @param params Parameters:
 *            id: command ID (optional)
 *            buffer: target buffer pointer (mandatory)
 *            name: local variable name (mandatory)
 *            value: local variable value (mandatory)
 * @return Formatted input command string
 */
WeeChatProtocol.formatLocalvarSet = function(params) {
    var defaultParams = {
        id: null
    };
    var parts = [];
    ...
    params = WeeChatProtocol._mergeParams(defaultParams, params);
    parts.push(params.buffer);
    parts.push('/buffer set localvar_set_' + params.name + ' ' + params.value);
