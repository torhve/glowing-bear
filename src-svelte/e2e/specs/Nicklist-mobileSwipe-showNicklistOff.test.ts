...
test('swipe from right opens nicklist with content even when showNicklist is off', async () => {
    await setSettings(browserPage, { showNicklist: false, alwaysnicklist: false });
    ...
    await browserPage.locator('.mobile-nicklist-overlay').waitFor({ state: 'attached', timeout: 10000 });
    ...
    await expect(browserPage.getByTestId('nicklist-items')).toBeVisible({ timeout: 5000 });
...
test('tapping nicklist button opens nicklist with content when showNicklist is off', async () => {
    // Close any open overlay from previous test
