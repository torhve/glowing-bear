...
async function waitForScrollStable(page: import('@playwright/test').Page, timeout = 5000) {
...
test.describe('PageUp/PageDown Global Scroll', () => {
    ...
    test('PageUp should scroll chat up when focus is outside input', async ({ page }) => {
    ...
    test('PageDown should scroll chat down when focus is outside input', async ({ page }) => {
        // Send enough messages to have content
        for (let i = 0; i < 30; i++) {
            await irc.sendMessage('#glowing-bear', `pagedown-test-${Date.now()}-${i}`);
        }
    ...
    test('PageUp should still work when focus is in message input', async ({ page }) => {
        for (let i = 0; i < 20; i++) {
            await irc.sendMessage('#glowing-bear', `input-pgup-test-${Date.now()}-${i}`);
        }
        await page.waitForTimeout(2000);
    ...
    test('PageUp should work when focus is on nicklist', async ({ page }) => {
        for (let i = 0; i < 15; i++) {
            await irc.sendMessage('#glowing-bear', `nicklist-pgup-test-${Date.now()}-${i}`);
        }
        await page.waitForTimeout(2000);
    ...
    test('PageUp should not scroll when focus is in a native INPUT element', async ({ page }) => {
        for (let i = 0; i < 15; i++) {
            await irc.sendMessage('#glowing-bear', `input-skip-test-${Date.now()}-${i}`);
        }
        await page.waitForTimeout(2000);
    ...
    test('PageUp with Ctrl modifier should not scroll via global handler', async ({ page }) => {
        for (let i = 0; i < 15; i++) {
            await irc.sendMessage('#glowing-bear', `modifier-test-${Date.now()}-${i}`);
        }
        await page.waitForTimeout(2000);
