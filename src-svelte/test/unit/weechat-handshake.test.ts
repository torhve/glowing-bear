...
describe('Protocol handshake parsing', () => {
    it('parses handshake response with plain password method', () => {
        const binary = buildMessage('cbid1', [
            { type: 'inf', content: { key: 'password_hash_algo', value: 'plain' } }
        ]);
        const protocol = new Protocol();
        const result = protocol.parse(binary);
    ...
    it('parses handshake response with PBKDF2 password method', () => {
        const binary = buildMessage('cbid1', [
    ...
    it('parses handshake with multiple info items (algo + iterations + nonce)', () => {
        const binary = buildMessage('cbid1', [
            { type: 'inf', content: { key: 'password_hash_algo', value: 'pbkdf2+sha512' } },
...
describe('Protocol version info parsing', () => {
    it('parses version info with short_version and full_version', () => {
